...
fileName="../contracts/${verifierName}.sol"
...
# Create folder in public/circuits for the circuit if it doesn't already exist, and
# copy over the necessary files
if [ ! -d "../public/circuits" ]; then
  mkdir ../public/circuits
fi
if [ ! -d "../public/circuits/$circuitName" ]; then
  mkdir ../public/circuits/$circuitName
fi
mv "${circuitName}_js/${circuitName}.wasm" "../public/circuits/${circuitName}/circuit.wasm"
mv circuit_final.zkey verification_key.json ../public/circuits/$circuitName
