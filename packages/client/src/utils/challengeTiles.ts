...
function generateChallengeTilesFromQuarter(hitTilesSet: [number, number][][]) {
...
export const challengeTilesOffsetList = generateChallengeTilesFromQuarter([
  [[1, 0], [2, 0], [3, 0], [4, 0]],
  [[1, 0], [2, 0], [3, -1], [4, -1]],
  [[1, 0], [2, -1], [3, -1], [4, -2]],
  [[1, -1], [2, -1], [3, -2], [4, -3]],
...
export function challengeTilesOffsetListToString(offsetsList: [number, number][][]) {
  return offsetsList.map(offsets => (
    `[${offsets.map((offset, index) => (
      `[int16(${offset[0]}), ${offset[1]}]${index < offsets.length - 1 ? ', ' : ''}`
    )).join('')}]`
  )).join(',\n');
}
...
export function calcPositionFromChallengeTiles(
  ...
  const matchingOffsets = challengeTilesOffsetList.find(offsets => {
