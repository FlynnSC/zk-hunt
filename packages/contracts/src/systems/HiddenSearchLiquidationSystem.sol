...
contract HiddenSearchLiquidationSystem is System {
  ...
  uint256 constant LIQUIDATION_PERIOD = 5; // Seconds after the end of the response period
  ...
  function executeTyped(
    ...
    require(
      block.timestamp > hiddenChallenge.creationTimestamp + RESPONSE_PERIOD,
      "Response period has not elapsed"
    );

    require(
      block.timestamp < hiddenChallenge.creationTimestamp + RESPONSE_PERIOD + LIQUIDATION_PERIOD,
